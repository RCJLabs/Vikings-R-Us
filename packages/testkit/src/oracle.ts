 ...
 * perceive and has been taught, let an unforged saga tally narrow them, apply
...
export function oracleSolve(fields: readonly Field[], ctx: DayCtx): OracleResult {
  ...
  // A saga tally narrows the worlds (it outranks presumptions) unless no world allows a line, which is then
  // a lie to catch; one forgery sign seen voids the whole tally.
  ...
  for (const line of carved) {
    const agree = worlds.filter((t) => t[line.says?.fact as string] === line.says?.value);
